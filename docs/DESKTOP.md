...
2. Build the `.deb` with the signing private key.
3. Create a GitHub release and upload:
   - `ML-Auditor_<version>_amd64.deb`
   - `ML-Auditor_<version>_amd64.deb.sig`
   - `latest.json` (generated by Tauri in the bundle directory)

The signing private key is:

```
frontend/src-tauri/ml-auditor-updater.key
```

It is gitignored; keep it secret. The public key is committed in
`tauri.conf.json`.

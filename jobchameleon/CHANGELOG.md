...
- Resume tailoring now paraphrases instead of rewriting. The generator keeps the candidate's exact resume content and only rephrases to weave in JD keywords — certifications, experience entries, bullet points, achievements, education, and skills are never deleted or truncated. Keyword weaving targets 5+ occurrences across SUMMARY, EXPERIENCE, and SKILLS sections.
- All profile skills are mandatory. The generated resume must include every skill from the candidate's profile — the skill list is a mandatory set that cannot be omitted.
- Skills section uses a consistent formatting template: grouped under 3-6 category headers derived from the profile and JD, comma-separated within each category, with JD-relevant skills listed first.
- Cover letter follows a formal 4-paragraph template: opening (role + company + interest), body (2-3 project/experience mappings), skills match (mandatory skills mapped to JD requirements), and closing (call to action). Tone is formal, 200-300 words.
- Chromium EACCES fix: backend now detects and fixes execute permissions on Chromium binaries and sibling executables (chrome_crashpad_handler, chrome_sandbox, chrome-wrapper) before launch.
- DOCX parser improved: handles tabs, line breaks, and BadZipFile errors with actionable messages.
- Better ingest error messages: shows file extension and gives specific hints for .docx vs .pdf failures.
- LLM client: added env var fallback for model names (NVIDIA_MODEL, OPENAI_MODEL, etc.), skips chat_template_kwargs for Mistral models (not supported on NVIDIA NIM), added verbose logging throughout the call chain.
- Backend loads .env file from project root on startup via python-dotenv.
- API key auto-saves on successful validation in Settings.
- Deterministic fallback resume also preserves all content (no truncation on experience, certs, achievements, education).

...
def run(raw: str = "", pdf: str | None = None) -> C:
    ...
    _log.info("run step=ingestor provider=%s key_present=%s model=%s txt_len=%d", p, bool(k), _model, len(txt))
    ...
    try:
        ...
        if is_transient_llm_error(exc):
            _log.warning(
                "LLM call failed (%s, transient): %s - falling back to local parser",
                p, exc,
            )
            return _parse_local(txt)
        if p != "ollama":
...
def ingest(raw: str = "", pdf: str | None = None) -> C:
    _log.info("ingest START raw_len=%d pdf=%s", len(raw or ""), pdf)
    pdf_text = _document(pdf) if pdf else ""
    _log.info("ingest document extracted %d chars from %s", len(pdf_text), pdf)
    ...
    if not txt.strip():
        if pdf and not raw.strip():
            ...
            if suffix == ".docx":
                hint = (
                    " The file may not be a valid DOCX — try re-exporting it from "
                    "Word or Google Docs as DOCX, or paste the text instead."
                )
            elif suffix == ".pdf":
                hint = (
                    " If it's a scanned or image-only PDF, paste the résumé text instead."
                )
            raise ValueError(
                f"Could not read any text from the uploaded document ({suffix}).{hint}"
            )

...
async def actuate_job_opencli(job_id: str, manager, repo: Repository | None = None, service=None, job_store=None) -> None:
    """Drive the application in the user's *logged-in* browser via OpenCLI.

    Mirrors ``actuate_job`` but uses the OpenCLI bridge instead of the headless
    Chromium actuator, so ATSs that need sign-in / OAuth / email verification work
    against the user's real Chrome profile.
    """
    from automation import opencli_bridge

    repo = repo or get_repository()
    service = service or get_automation_service()
    job_store = job_store or get_job_runner()
    job = job_store.create("automation_fire_opencli", {"job_id": job_id})
    try:
        ...
        if not opencli_bridge.opencli_available():
            detail = "OpenCLI is not available — install the Browser Bridge extension and `npm i -g @jackwener/opencli`, then re-run."
            await manager.broadcast({
                "type": "agent",
                "event": "failed",
                "job_id": job_id,
                "msg": f"OpenCLI blocked for {job_id}: {detail}",
            })
            job_store.update(job.job_id, status="failed", error=detail)
            return
        ...
        await manager.broadcast({
            ...
            "event": "actuating",
            "job_id": job_id,
            "msg": f"Opening {lead.get('title','')} @ {lead.get('company','')} in your browser (OpenCLI)",
        })
        report = await opencli_bridge.apply_with_opencli(
            lead.get("url", ""),
            lead,
            resume_path=asset,
        )
        job_store.update(job.job_id, status="running", progress=85)
    except Exception as exc:
        logging.getLogger(__name__).warning('suppressed exception in backend/api/routers/automation.py:actuate_job_opencli: %s', exc)
        await manager.broadcast({
            "type": "agent",
            "event": "failed",
            "job_id": job_id,
            "msg": f"OpenCLI submission failed for {job_id}: {exc}",
        })
    ...
    if report.get("submitted"):
        ...
        await manager.broadcast({
            ...
            "event": "applied",
            "job_id": job_id,
            "msg": f"Application submitted for {job_id} via OpenCLI",
        })
    else:
        note = report.get("note") or "OpenCLI filled the form but did not confirm submission"
        job_store.update(job.job_id, status="failed", error=note)
        await manager.broadcast({
            "type": "agent",
            "event": "failed",
            "job_id": job_id,
            "msg": f"OpenCLI submission incomplete for {job_id}: {note}",
        })
...
def create_router(manager) -> APIRouter:
    ...
    @router.get("/automation/opencli/status")
    async def opencli_status_endpoint():
        from automation.opencli_bridge import opencli_status

        return await opencli_status()

    @router.post("/leads/{job_id}/apply/opencli")
    async def apply_opencli(
        job_id: str,
        bt: BackgroundTasks,
        repo: Repository = Depends(get_repository),
        service=Depends(get_automation_service),
    ):
        from automation.opencli_bridge import opencli_available
        ...
        bt.add_task(actuate_job_opencli, job_id, manager, repo, service)
        return {"status": "opencli_firing", "job_id": job_id}

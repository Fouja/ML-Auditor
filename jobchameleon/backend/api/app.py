...
from api.webui import render_console
...
def create_app(
    *,
    ...
    # ML-Auditor container integration adds the compose service name.
    allowed_hosts = (
        os.environ.get("JC_ALLOWED_HOSTS")
        or os.environ.get("JHM_ALLOWED_HOSTS", "")
    ).split(",")
    base_hosts = ["localhost", "127.0.0.1", "[::1]", "::1", "ml-auditor", "mlauditor", "frontend", "web"]
    app.add_middleware(
        TrustedHostMiddleware,
        allowed_hosts=[h for h in [*base_hosts, *allowed_hosts] if h],
    )
    ...
    async def console():
        return HTMLResponse(content=render_console(token_getter()))

...
def _fallback_package(profile: dict, lead: dict, template: str = "") -> _DocPackage:
    selected = _rank_projects(profile, lead, limit=20)
    ...
    for e in profile.get("exp", []):
        ...
        for bullet in _experience_bullets(e, lead, prioritized_skills):
            exp_block += f"- {bullet}\n"
        exp_lines.append(exp_block)

    cert_lines = _line_items(certs, 20)
    achv_lines = _line_items(achievements, 20)
    edu_lines = _line_items(education, 10)

...
* Logging writes structured JSON lines and also ships to Logstash TCP
"""
...
from config.logging_handlers import JSONFormatter, TcpJsonLogHandler  # noqa: E402
...
# Client logs (web view inside the desktop app) are written here so the sidecar
# can persist them alongside its own logs.
CLIENT_LOG_DIR = Path(
    os.environ.get("CLIENT_LOG_DIR", DATA_DIR / "logs" / "clients")
)
...
# -----------------------------------------------------------------------------
# Monitoring — disable external APM/Sentry in desktop builds
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# Logging — JSON lines to local file + Logstash TCP for Kibana
# -----------------------------------------------------------------------------

# Send directly to the Logstash TCP input running on the host. If Logstash is
# not running the handler silently drops the line so the app keeps working.

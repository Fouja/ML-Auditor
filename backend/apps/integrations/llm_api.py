...
def update_llm_configuration(request, config_id: str, payload: LLMConfigurationUpdateSchema):
    ...
    try:
        ...
        if config.provider == 'openai':
            ...
            if response.status_code == 200:
                ...
                return JsonResponse({"error": f"Erreur API: {response.status_code}"}, status=400)
            ...
            if response.status_code == 200:
                ...
                return JsonResponse({"error": "Impossible de se connecter à Ollama"}, status=400)
